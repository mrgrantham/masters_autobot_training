...
# ax1.figure(num=None, figsize=(12, 10), dpi=200, facecolor='w', edgecolor='k')
...
train_loss, = ax1.plot(train_log['#Seconds'], train_log['TrainingLoss'], color='red',  alpha=.5)
test_loss, = ax1.plot(test_log['#Seconds'], test_log['TestLoss'], linewidth=2, color='green')
ax1.set_ylim(ymin=0, ymax=1)
ax1.set_xlabel('Time Elapsed (sec)', fontsize=15)
ax1.set_ylabel('Loss', fontsize=15)
ax1.tick_params(labelsize=15)
#Plotting test accuracy
ax2 = ax1.twinx()
# test_accuracy, = ax2.plot(test_log['#Iters'], test_log['TestAccuracy'], linewidth=2, color='blue')
test_accuracy, = ax2.plot(test_log['#Seconds'], test_log['TestAccuracy'], linewidth=2, color='blue')
