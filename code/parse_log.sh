...
# Generating
echo '#Seconds Iters TestAccuracy TestLoss'> $LOG.test
paste aux4.txt aux0.txt aux1.txt aux2.txt | column -t >> $LOG.test
...
grep ', loss = ' $1 >> aux.txt
grep 'Iteration ' aux.txt | sed  's/.*Iteration \([[:digit:]]*\).*/\1/g' > aux0.txt
#grep ', loss = ' $1 | awk '{print $9}' > aux1.txt #old code
...
# Generating
echo '#Seconds Iters TrainingLoss LearningRate'> $LOG.train
paste aux3.txt aux0.txt aux1.txt aux2.txt | column -t >> $LOG.train
